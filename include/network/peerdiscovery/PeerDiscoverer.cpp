...
namespace NAMESPACE_NAME {
    ...
    void PeerDiscoverer::cleanup_peer_listeners() {
        std::lock_guard<std::recursive_mutex> lock(premature_shutdown_listener_lock);
        for (auto ptr = premature_shutdown_listener_list.begin();
             ptr != premature_shutdown_listener_list.end(); ptr++) {
            PeerListener *peerListener = *ptr;
            peerListener->stop();
            delete (peerListener);
        }
            ...
            if (peerDiscoveryReplyPacket->is_valid()) {
                ...
                if (ptr != stream_ids_to_search_for.end()) {
                    if (listener_already_exists(stream_id, sender.sin_addr,
                                                peerDiscoveryReplyPacket->get_port_number())) {
                        delete (peerDiscoveryReplyPacket);
                        return;
                    }
                    StreamPacketDataReceiver *streamPacketDataReceiver = ptr->second.first;
                    auto peer_listener = new PeerListener(this, stream_id, sender.sin_addr,
                                                          peerDiscoveryReplyPacket->get_port_number(),
                                                          streamPacketDataReceiver);
    ...
    void PeerDiscoverer::send_peer_discovery_reply(PeerSender *peerSender) {
        auto peerDiscoveryReplyPacket = new PeerDiscoveryReplyPacket(peerSender->get_tcp_port(),
                                                                     peerSender->get_stream_id());
